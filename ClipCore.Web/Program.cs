...
try
{
    ...
    if (checkTask.Wait(TimeSpan.FromSeconds(6))) // 1s buffer over 5s timeout
    {
        ...
        builder.Configuration.AddSystemsManager("/clipcore");
    }
    else
    {
...
if (configLoaded)
{
    if (builder.Environment.IsDevelopment())
    {
        ...
        builder.Services.AddSingleton<IDbContextFactory<AppDbContext>>(sp =>
        {
            ...
            return new ClipCore.Infrastructure.Data.DbContextFactoryWrapper(factory);
        });
    }
    else
    {
...
// ExternalProductRepository removed
...
builder.Services.AddScoped<ClipCore.Web.Services.OrderFulfillmentService>();
