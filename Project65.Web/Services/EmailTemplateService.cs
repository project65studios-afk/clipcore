...
public class EmailTemplateService
{
    ...
    public EmailTemplateService(ISettingsRepository settingsRepository, NavigationManager navigationManager)
    {
    ...
    {
        var logoUrl = await _settingsRepository.GetValueAsync("BrandLogoUrl");
        var absoluteLogoUrl = !string.IsNullOrEmpty(logoUrl) 
            ? _navigationManager.ToAbsoluteUri(logoUrl).ToString() 
            : null;
        ...
        var headerContent = !string.IsNullOrEmpty(absoluteLogoUrl)
            ? $@"<img src=""{absoluteLogoUrl}"" alt=""{storeName}"" style=""max-height: 60px; width: auto;"" />"
...
<body style=""margin: 0; padding: 0; background-color: #0d0d0d; font-family: 'Inter', -apple-system, sans-serif; color: #ffffff;"">
    <div style=""max-width: 600px; margin: 0 auto; background-color: #1a1a1a; border-radius: 12px; overflow: hidden; margin-top: 40px; margin-bottom: 40px; border: 1px solid #333333;"">
        ...
        <div style=""padding: 40px;"">
            ...
            <p style=""font-size: 16px; line-height: 1.6; color: #e0e0e0;"">Thank you for your purchase from {storeName}! Your order has been processed successfully.</p>
